...
export function PortfolioGalleryGrid({ items }: PortfolioGalleryGridProps) {
  const pairs = Array.from({ length: Math.ceil(items.length / 2) }, (_, index) =>
            ...
            <div key={pairIndex} className="grid grid-cols-2 gap-2.5">
              {pair.map((item, itemIndex) => (
                <article
                  ...
                  className="relative aspect-[220/300] rounded-[5px] bg-brand-black/10"
                >
                  ...
                  <span className="absolute top-[-11px] left-1/2 flex h-6 w-[68px] -translate-x-1/2 items-center justify-center gap-2.5 rounded-[100px] bg-brand-red px-4 py-1.5 font-body text-xs leading-4 font-normal tracking-normal text-white uppercase">
                    {item.label}
                  </span>
                </article>
              ))}
